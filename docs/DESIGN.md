...
设计沿两次责任交接展开：业务方向通知服务提交请求，通知服务再向供应商发出请求。接收阶段要确认任务是否保存，执行阶段要判定响应，恢复阶段要处理没有明确结果的尝试。先确定这些边界，再选择模块与组件，避免从“异步通知”直接推导出消息平台或工作流系统。
...
供应商差异首先表现为 URL、Header 和 Body，因此通用层接收已构造好的请求。正文限定为有大小上限的 UTF-8 文本，支持 JSON、XML 和表单等内容。字段转换、业务码解析和动态认证会把服务带入各供应商的业务协议，不纳入第一版，也不提供通用脚本执行。文件或流式上传、WebSocket 则需要不同的资源与生命周期管理。

严格顺序、跨系统事务和业务结果对账也不属于第一版。顺序处理需要决定一条失败通知是否阻塞后续事件，对账需要供应商的查询与业务标识支持，这些前提不能由投递服务自行假设。库存场景按交易后的同步理解，交易时的库存预占需要独立的一致性约定。
...
遇到结果不确定的请求，停止重试会增加漏送风险，继续重试则可能重复执行。方案选择通过重试提高送达机会，并把重复副作用的处理责任明确交给业务契约。数据库锁和领取令牌约束本地执行权，无法把本地提交与供应商的业务操作合成一个事务。
...
在既定的 MySQL 约束下，关系事务与唯一约束可以同时承担接收去重、任务领取和结果更新，因此任务与调度状态集中保存。纯内存队列无法满足重启恢复；独立消息队列可以分离消费负载，却会增加消息发布、消费确认以及与数据库保持一致的责任，也不能消除供应商重复执行的窗口。

第一版选择承担轮询、频繁更新和历史记录带来的数据库负载，换取更少的运行组件和更直接的排障路径。索引与保留策略属于必要设计，容量测试则在实际规模要求出现时开展，不作为 MVP 的默认交付门槛。技术取舍依据当前职责和失败路径，不将组件数量作为可靠性的证明。
...
等待任务也必须有明确的终止入口：超过有效期时转为过期失败，目标删除或撤权时按目标失效处理。候选查询不能让已经失效的目标占满结果后再逐个跳过，否则有效目标可能一直得不到执行机会。这一要求应由调度与目标校验共同保证。
...
摘要编码必须保留 Header 名称、值列表及各字段的结构边界，不能把不同层次的字符串直接平铺。大小写归一化后出现同名 Header 时，需要明确拒绝或按稳定规则合并，不能依赖 map 遍历顺序。唯一约束只能保证键不重复，不能替代请求内容比较的正确性。
...
验证围绕能够推翻设计判断的反例组织：相同输入是否稳定去重，不同输入是否被误认，失效执行者是否仍能写回，以及配置变化是否阻塞正常任务。状态、事务和真实请求记录提供判断依据，单次接口返回成功只支持该次行为。

MVP 使用少量代表性用例验证主流程与关键故障，不要求穷举所有协议、搭建故障平台或达到覆盖率指标。发现可复现的问题后，优先修正直接相关的机制并补充针对性回归，不通过扩大架构或反复全量测试代替定位。

测试环境本身也要遵守数据边界。数据库用例必须与运行数据隔离，不同测试包的清理不能相互影响。验证脚本和运行说明的可复现性包含安全的数据准备与清理，不能以测试通过为由删除已有通知。
